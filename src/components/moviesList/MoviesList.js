...
export default function MoviesList({ movie, count, genres, switchTheme }){
    return(
        count < 15 ?
            <div className="movie__list-item" style={{marginRight: "20px"}}>
                <Link to={
                    ...
                    {count % 2 === 0 ?
                        ...
                        <div><img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt="movie-icon"
                                  style={{width: '100%', height: '245px'}}/></div>}
                    <div className="list__item-container">
                        ...
                        <div
                            className="list__item-description">{movie.overview.length > 30 ? movie.overview.substring(0, movie.overview.indexOf('.')) + "..." : movie.overview}</div>
                    </div>
                </Link>
                <hr/>
            ...
            <div className="movie__list-item">
                <Link to={
                    {
                        pathname: `/movies/${movie.id}`,
                        state: {movie, genres, switchTheme}
                    }
                }>
                    {count % 2 === 0 ?
                        <div><img src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt="movie-icon"
